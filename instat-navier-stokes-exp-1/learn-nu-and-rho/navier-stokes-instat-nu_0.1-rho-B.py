...
def read_traindata_for_iteration_and_file(i, k):
    ...
    path = "D:\Dokumente\Studium\Bachelorarbeit\data\\nav_sto_stationary\data_lvl_3\\sol_channel_ilupp{}_{}.vtu".format(j,k)
...
model.compile("adam", lr=0.0001, external_trainable_variables=[nu, rho])
losshistory, train_state = model.train(epochs=10000, callbacks=[variable], display_every=100, disregard_previous_best=True)
...
def plot_pred_for_gif(i):
    ...
    # Für p
    plt.figure()
    cntr2 = plt.tricontourf(x_at_t, y_at_t, p_pred_at_t, levels=80, cmap="rainbow", vmin=min_p_pred, vmax=max_p_pred)
    plt.colorbar(cntr2)
    ...
    plt.figure()
    cntr2 = plt.tricontourf(x_at_t, y_at_t, p_true_at_t, levels=80, cmap="rainbow", vmin=min_p_true, vmax=max_p_true)
    plt.colorbar(cntr2)
